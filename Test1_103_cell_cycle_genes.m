...
td = 1:5;
PX0 = csvread('px0_103_cell_cycle.csv');
PX1 = csvread('px1_103_cell_cycle.csv');
PX(:,:,1)=PX0;
PX(:,:,2)=PX1;
Gene_names = importdata('gene_names_103_cell_cycle.csv');
Gene_names = Gene_names(2:end);
Tf_no = csvread('number_of_regulators_103_cell_cycle.csv');
n = size(PX,1);
m = size(PX,2);
...
Regulator_name = Gene_names(Result_test1(:,1));
Target_name = Gene_names(Result_test1(:,2));
Result_table = [Regulator_name, Target_name, num2cell(Result_test1)];
T = cell2table(Result_table,'VariableNames',{'Source','Target','Source_No','Target_No','Function','Time_delay','Lik_Ratio','p_value'});
writetable(T,'test1_result_103_cell_cycle.csv');